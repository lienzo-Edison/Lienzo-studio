  ...
  return (
    <main className="min-h-screen bg-[#e6e1d5] text-[#0f1418]">
      ...
      <section className="mt-12">
        ...
        <div className="mt-6 overflow-x-auto rounded-3xl border border-black/15 bg-[#f6f1e7]/45">
          <table className="min-w-[800px] w-full text-left text-sm">
            <thead className="bg-black/[0.035] text-xs font-bold uppercase tracking-[0.12em] text-black/50">
              <tr>
              ...
              {accounts?.map((account) => (
                <tr key={account.id} className="border-t border-black/10">
                  <td className="px-5 py-5">
                    ...
                    <p className="mt-1 text-black/55">{account.email}</p>
                  </td>
                    ...
                    {account.role === "admin" ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        <form action={changeAccountType}>
                          ...
                          <button className="rounded-full border border-black/20 px-3 py-2 text-xs font-bold uppercase tracking-[0.1em] transition hover:border-black hover:bg-black/[0.04]">
                            Mark new
                          </button>
                        </form>
                        <form action={assignSampleProject}>
                          <input type="hidden" name="userId" value={account.id} />
                          <button className="rounded-full bg-[#0f1418] px-3 py-2 text-xs font-bold uppercase tracking-[0.1em] text-[#f6f1e7] transition hover:bg-black/80">
          ...
          {projects?.map((project) => {
            ...
            return (
              <form
                ...
                className="rounded-3xl border border-black/15 bg-[#f6f1e7] p-6 shadow-[0_20px_60px_-52px_rgba(0,0,0,0.38)]"
              >
                ...
                <p className="mt-1 text-sm text-black/60">{project.service_label}</p>
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-2 text-xs font-bold uppercase tracking-[0.12em]">
                    ...
                    <select
                      ...
                      className="rounded-xl border border-black/15 bg-[#e6e1d5]/65 px-3 py-3 text-sm font-medium normal-case tracking-normal"
                    >
                ...
                <button className="mt-5 rounded-full border border-black/20 px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] transition hover:border-black hover:bg-black/[0.04]">
                  Save status
                </button>
              </form>
            );
